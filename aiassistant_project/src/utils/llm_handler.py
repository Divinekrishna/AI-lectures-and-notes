        ...
        try:
            ...
            for msg in messages:
                ...
                if role == 'system':
                    ...
                    # For assistant messages, treat as HumanMessage for now
                    # LangChain handles conversational context differently
                    langchain_messages.append(HumanMessage(content=content))
            ...
            if "RESOURCE_EXHAUSTED" in error_msg or "quota" in error_msg.lower():
                return "⚠️ API quota exceeded. Please wait a moment and try again, or upgrade your Gemini API plan at https://ai.google.dev/pricing"
...
Answer:"""
    ...
    def find_relevant_resources(self, query: str, available_resources: List[str]) -> List[str]:
        """Find relevant resources based on query using LangChain."""
        resources_text = "\n".join(available_resources)
        
        template = """Given the following resources, identify which ones are most relevant to the query.
...
Please list the most relevant resources (one per line):"""
        ...
        try:
            self._rate_limit()
            result = chain.invoke({"resources": resources_text, "query": query})
            return [r.strip() for r in result.content.split('\n') if r.strip()]
        except Exception as e:
            error_msg = str(e)
            print(f"Error in find_relevant_resources: {error_msg}")
            return []
